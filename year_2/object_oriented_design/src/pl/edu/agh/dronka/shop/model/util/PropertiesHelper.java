...
public class PropertiesHelper {
	...
	public static Map<String, Object> getPropertiesMap(Item item) {
		...
		switch (item.getCategory()) {
			case FOOD -> {
				propertiesMap.put("Data przydatności do spożycia", item.getExpiryDate());
			}
			case BOOKS -> {
				propertiesMap.put("Liczba stron", item.getNoPages());
				propertiesMap.put("Twarda oprawa", item.isHardCover());
			}
			case MUSIC -> {
				propertiesMap.put("Gatunek muzyczny", item.getMusicGenre().getDisplayName());
				propertiesMap.put("Dołączone video", item.isVideoIncluded());
			}
			case ELECTRONICS -> {
				propertiesMap.put("Mobilny", item.isMobile());
				propertiesMap.put("Gwarancja", item.isWarranty());
			}
			case SPORT -> {}
		}
